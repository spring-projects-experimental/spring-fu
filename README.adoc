...
Spring Fu is an experimental project designed to bring Kotlin Spring support to the next level.

It introduces Kofu (for **Ko**tlin and **fu**nctional) configuration which allows to configure your
application with Kotlin DSL and lambdas instead of annotations, Coroutines native support and
GraalVM native image support to compile your application to a native executable.

It is not intended to be used in production, but rather get feedback and contributions from the
community in order to reach a point where it's features can be integrated as part of existing
...
different from regular auto-configuration. It has following characteristics:
...
The `application { }` Kotlin DSL allows you to configure your application explicitly. Each custom block like `configuration` or `server` is in fact a more high level `beans {}` block with a custom DSL provided for easy configuration. Since this configuration is code, you can use any kind of custom programmatic bean registration without having to implement your own `@Conditional` annotation.
...
Declare application event Listeners in order to run tasks when `ApplicationContextEvent` like `ApplicationReadyEvent` are emitted.
...
Require `org.springframework.boot:spring-boot-starter-json` dependency (included by default in `spring-boot-starter-webflux`).

This DSL configures a https://github.com/FasterXML/jackson[Jackson] JSON codec on WebFlux server and client.
