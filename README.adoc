...
Spring Fu is an incubator for new Kotlin + Spring features. It introduces a consistent set of features
intended to bring Kotlin support in Spring to the next level. The main one is an alternative way of
configuring Spring Boot applications with Kotlin DSL and lambdas instead of annotations:
Kofu (for **Ko**tlin and **fu**nctional) configuration. Other features like Coroutines or
GraalVM support are also included.
