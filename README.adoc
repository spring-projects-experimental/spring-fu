Spring Fu is an incubating https://kotlinlang.org/[Kotlin] micro-framework that makes it easy to create lightweight https://spring.io/projects/spring-framework[Spring]-powered applications with <<functional-configuration,functional APIs>> instead of annotations.
...
```
It is a thin but opinionated layer on top of a subset of Spring Framework, Spring Data, Spring Security and other Spring projects that provides an alternative programming model to https://github.com/spring-projects/spring-boot[Spring Boot] to develop applications in Kotlin.
