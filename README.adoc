Spring Fu is an incubating https://kotlinlang.org/[Kotlin] micro-framework that makes it easy to create lightweight https://spring.io/projects/spring-framework[Spring]-powered applications leveraging <<functional-configuration,functional configuration>>.

== Overview

Spring Fu is a thin but opinionated layer on top of a subset of Spring Framework, Spring Data, Spring Security and other Spring projects that provides an alternative programming model than https://github.com/spring-projects/spring-boot[Spring Boot] to develop applications in Kotlin. No blocking web/persistence support is provided since https://github.com/Kotlin/kotlinx.coroutines/blob/master/coroutines-guide.md[coroutines] provides support for imperative programming.
...
* https://github.com/sdeleuze/spring-fu/issues/34[JAR minimization] in order to have minimal classpath and deploy just what you use
