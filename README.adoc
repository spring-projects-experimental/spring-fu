...
Spring Fu is an incubator for *Kofu* (*Ko* for Kotlin, *fu* for functional), a set of
...
to configure Spring Boot  explicitly with code in a declarative way with great
discoverability thanks to auto-complete.
...
It provides fast startup and low memory consumption.
