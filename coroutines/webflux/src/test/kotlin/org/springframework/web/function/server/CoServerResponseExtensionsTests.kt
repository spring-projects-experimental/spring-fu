...
class CoServerResponseExtensionsTests {
    ...
    @Test
    fun `bodyAndAwait with suspendable function parameter`() {
        val response = mockk<ServerResponse>()
        val builder = mockk<ServerResponse.BodyBuilder>()
        val supplier: suspend () -> String = mockk()
        every { builder.body<String>(ofType<Mono<String>>()) } returns Mono.just(response)
        runBlocking {
            builder.bodyAndAwait(supplier)
        }
        verify {
            builder.body<String>(ofType<Mono<String>>())
        }
    }
