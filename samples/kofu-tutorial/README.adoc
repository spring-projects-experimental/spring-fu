...
We just created a new configuration and put the repositories into the context with `beans{}`. Then we enabled the new configuration.
...
We are asking spring to give us an implementation of a `Datasource`. Pay attention that `spring-boot-starter-jdbc` doesn't create magically a datasource
...
We split the routing declaration from the routing handling. The route is declared into a `configuration`, while we the
...
contrast of Spring, where you have to seatch for the right annotation that disables what you don't need for the test.
...
We removed the repository mock and enabled the real configurations. Everything else remains untouched. The test just became
an integration test.

== Exposing an HTTP API

Writing an HTTP API is pretty much the same of we saw up to now. It only changes the function that we call when returning a
`ServerResponse`.
...
There are two way to deal with configuration properties with KoFu inside the `configuration{}` DSL lambda function:
...
put some properties in `application.properties`
...
The `HtmlHandler` receives the addition parameter `private val properties: BlogProperties` that is put into the Spring context
...
Yet, many important integration, like https://liquibase.org/[Liquibase], are still missing. That's not a big deal, has we can easily work around it.
