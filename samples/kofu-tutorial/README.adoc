...
If now you start your application and try to run `curl http://localhost:8080/api/article -v`, you will receive a `406` error.

Do you rembember that KoFu doesn't start things automatically? No one told to Spring how to convert back the `RenderdArticle` and
...
Let's write some tests on the API
