...
We are asking spring to give us an implementation of a `Datasource`. Pay attention that `spring-boot-starter-jdbc` doesn't inject a datasource
like it does with regulat Springboot. We have to declare it explicitly as it follows:
...
We declare `RenderedUser` and `RenderedArticle` classes in order to simplify the usage in the mustache templates.
