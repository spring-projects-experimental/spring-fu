...
public class EmbeddedDataSourceConfigurationInitializer implements ApplicationContextInitializer<GenericApplicationContext> {
    ...
    public void initialize(GenericApplicationContext context) {
        if (
            context
                .getBeanFactory()
                .getBeanNamesForType(EmbeddedDataSourceConfiguration.class).length == 0
        ) {
            context.registerBean(
                "dataSource",
                EmbeddedDatabase.class,
                () -> new EmbeddedDataSourceConfiguration().dataSource(this.dataSourceProperties),
                def -> def.setDestroyMethodName("shutdown")
            );
        }
    }
}
